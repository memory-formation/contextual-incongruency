...
load([save_root '\ERPs_smoth_down_enc_Exp1.mat'])
load ([save_root '\reject_index_Exp1.mat'])
load ( [save_root 'beh_Exp1.mat'])
...
for s=1:25
    ...
    display (['Stability for Subject Number: ' int2str(s) '/' int2str(25)]);

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%% S11 with S22   %%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %     [I II] = find (reject_index_S11(s,:).*reject_index_S22(s,:) > 0);
    II = 1:99;
    for i=1:length(II)

        [R,P] = corr (DATA_study1_SIMIdown(s).S11(:,time_window,(II(i))),DATA_study1_SIMIdown(s).S22(:,time_window,(II(i))),'type','pearson');
        SIMI_result_general_Exp1(s).S11_S22(:,i) = fisherz(diag(R));
    end
    %     SIMI_result_general_Exp1(s,1,:) = mean(FF,2)';
    ...
    for i=1:length(II)
        ...
        [R,P] = corr (DATA_study1_SIMIdown(s).S22(:,time_window,(II(i))),DATA_study1_SIMIdown(s).S33(:,time_window,(II(i))),'type','pearson');
        SIMI_result_general_Exp1(s).S22_S33(:,i) = fisherz(diag(R));
    end
    %     SIMI_result_general_Exp1(s,2,:) = mean(FF,2)';
    ...
    for i=1:length(II)
        ...
        [R,P] = corr (DATA_study1_SIMIdown(s).S33(:,time_window,(II(i))),DATA_study1_SIMIdown(s).S44(:,time_window,(II(i))),'type','pearson');
        SIMI_result_general_Exp1(s).S33_S44(:,i) = fisherz(diag(R));
    end
    %     SIMI_result_general_Exp1(s,3,:) = mean(FF,2)';
    ...
    for i=1:length(II)
        ...
        [R,P] = corr (DATA_study1_SIMIdown(s).S44(:,time_window,(II(i))),DATA_study1_SIMIdown(s).S55(:,time_window,(II(i))),'type','pearson');
        SIMI_result_general_Exp1(s).S44_S55(:,i) = fisherz(diag(R));
    end
    %     SIMI_result_general_Exp1(s,4,:) = mean(FF,2)';
    ...
    for i=1:length(II)
        ...
        [R,P] = corr (DATA_study1_SIMIdown(s).S55(:,time_window,(II(i))),DATA_study1_SIMIdown(s).S66(:,time_window,(II(i))),'type','pearson');
        SIMI_result_general_Exp1(s).S55_S66(:,i) = fisherz(diag(R));
    end
    %     SIMI_result_general_Exp1(s,5,:) = mean(FF,2)';
end

clearvars -except SIMI* reject*
...
for s = 1:25
    ...
    a = ind_S1S2(s,:);
    [U UU] = find(a > 0);
    SIMI_exp1(s,1,:) = mean(SIMI_result_general_Exp1(s).S11_S22(:,UU),2)';

    a = ind_S2S3(s,:);
    [U UU] = find(a > 0);
    SIMI_exp1(s,2,:) = mean(SIMI_result_general_Exp1(s).S22_S33(:,UU),2)';

    a = ind_S3S4(s,:);
    [U UU] = find(a > 0);
    SIMI_exp1(s,3,:) = mean(SIMI_result_general_Exp1(s).S33_S44(:,UU),2)';

    a = ind_S4S5(s,:);
    [U UU] = find(a > 0);
    SIMI_exp1(s,4,:) = mean(SIMI_result_general_Exp1(s).S44_S55(:,UU),2)';

    a = ind_S5S6(s,:);
    [U UU] = find(a > 0);
    SIMI_exp1(s,5,:) = mean(SIMI_result_general_Exp1(s).S55_S66(:,UU),2)';
...
for s = 1:25
    ...
    [row_1, col_1] = find(isnan(SIMI_result_general_Exp1(s).S11_S22(:,:)));
    [row_2, col_2] = find(isnan(SIMI_result_general_Exp1(s).S22_S33(:,:)));
    [row_3, col_3] = find(isnan(SIMI_result_general_Exp1(s).S33_S44(:,:)));
    [row_4, col_4] = find(isnan(SIMI_result_general_Exp1(s).S44_S55(:,:)));
    [row_5, col_5] = find(isnan(SIMI_result_general_Exp1(s).S55_S66(:,:)));
...
for s = 1:25
    ...
    %% Stability data
    SIMI_S11_S22 = SIMI_result_general_Exp1(s).S11_S22(:,ind_S11);
    SIMI_S22_S33 = SIMI_result_general_Exp1(s).S22_S33(:,ind_S22);
    SIMI_S33_S44 = SIMI_result_general_Exp1(s).S33_S44(:,ind_S33);
    SIMI_S44_S55 = SIMI_result_general_Exp1(s).S44_S55(:,ind_S44);
    SIMI_S55_S66 = SIMI_result_general_Exp1(s).S55_S66(:,ind_S55);
