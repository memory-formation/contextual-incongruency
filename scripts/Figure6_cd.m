...
load([save_root '\Erps_0.1_20hz_eventoffset.mat'])
...
for isubj=1:29
    ...
    SIMI_general(isubj).offset_1=zeros(size(Data_offset_event{isubj}));
    ...
    for ichan=1:29
        ...
        for itrial=1:size(Data_offset_event{isubj},3)
            SIMI_general(isubj).offset_1(ichan,:,itrial)=smoothdata(Data_offset_event{isubj}(ichan,:,itrial),'movmean',N);
        end
...
for s=1:29
    ...
    %%%%%%%% offset1 %%%%%%%%
    for i=1:size(SIMI_general(s).offset_1,3)
        for j=1:29
            SIMI_general_down(s).offset_1(j,:,i)=downsample(squeeze(SIMI_general(s).offset_1(j,52:end,i)),5); % downsample and removal of the baseline
...
%% general simi analysis
% first analysis without seperating beh performance
...
for s=1:29
    ...
    for i=1:80
        SIMI_result_general(s).S11_offset_1(:,:,i) = corr_matrix(SIMI_general_down(s).offset_1(:,:,i),SIMI_general_down(s).S11(:,:,i));
    end 
    ...
    for i=1:80
        SIMI_result_general(s).S22_offset_1(:,:,i) = corr_matrix(SIMI_general_down(s).offset_1(:,:,i),SIMI_general_down(s).S22(:,:,i));
    end 
    ...
    for i=1:80
        SIMI_result_general(s).S33_offset_1(:,:,i) = corr_matrix(SIMI_general_down(s).offset_1(:,:,i),SIMI_general_down(s).S33(:,:,i));
    end 
    ...
    for i=1:80
        SIMI_result_general(s).S44_offset_1(:,:,i) = corr_matrix(SIMI_general_down(s).offset_1(:,:,i),SIMI_general_down(s).S44(:,:,i));
    end 
    
end


%% LMM on RSA event offset
...
ind_S11off1_long = reject_index_S11.*reject_index_offset1;
ind_S22off1_long = reject_index_S22.*reject_index_offset1;
ind_S33off1_long = reject_index_S33.*reject_index_offset1;
ind_S44off1_long = reject_index_S44.*reject_index_offset1;

Mem = zeros(0);Sub_ID = zeros(0);Seq = zeros(0); Cog = zeros(0);Y_all = zeros(0);Recl=zeros(0);
for s=1:29
    disp(s)
    ind_S11=ind_S11off1_long(s,:)==1 & ind_mem(s,:) ~= 0 & ind_subcong(s,:)~=0;
    ind_S22=ind_S22off1_long(s,:)==1 & ind_mem(s,:) ~= 0 & ind_subcong(s,:)~=0;
    ind_S33=ind_S33off1_long(s,:)==1 & ind_mem(s,:) ~= 0 & ind_subcong(s,:)~=0;
    ind_S44=ind_S44off1_long(s,:)==1 & ind_mem(s,:) ~= 0 & ind_subcong(s,:)~=0;
    ...
    %% SIMI data
    SIMI_S11 = SIMI_result_general(s).S11_offset_1(:,:,ind_S11);
    SIMI_S22 = SIMI_result_general(s).S22_offset_1(:,:,ind_S22);
    SIMI_S33 = SIMI_result_general(s).S33_offset_1(:,:,ind_S33);
    SIMI_S44 = SIMI_result_general(s).S44_offset_1(:,:,ind_S44);
...
%% Prepare RSA result
Simi_all = reshape(Y_all,[200,200,size(Y_all,2)]);

tw = 172:182; % [-50,50] of peak
...
%%
t_map_real = zeros(2,200); 
beta_map_real = zeros(2,200); 
p_map_real = zeros(2,200); 
CI_low_map_real = zeros(2,200); 
CI_high_map_real = zeros(2,200); 
...
for i_time =1:size(Ydata_alltp_rs,1)
    ...
    tbl_time = table(Ydata_alltp_rs(i_time,:)',Sub_ID(:),Seq(:),'VariableNames',{'SIMI','Subject_ID','Seq'});
    lme_time = fitlme(tbl_time,'SIMI~Seq+(1|Subject_ID)');
...
for fig=6
    %% Figure 6c
    for s = 1:29
        RSA_offset1_all_S11(:,:,s) = mean(SIMI_result_general(s).S11_offset_1(:,:,reject_index_offset1(s,:)==1&reject_index_S11(s,:)==1),3);
        RSA_offset1_all_S22(:,:,s) = mean(SIMI_result_general(s).S22_offset_1(:,:,reject_index_offset1(s,:)==1&reject_index_S22(s,:)==1),3);
        RSA_offset1_all_S33(:,:,s) = mean(SIMI_result_general(s).S33_offset_1(:,:,reject_index_offset1(s,:)==1&reject_index_S33(s,:)==1),3);
        RSA_offset1_all_S44(:,:,s) = mean(SIMI_result_general(s).S44_offset_1(:,:,reject_index_offset1(s,:)==1&reject_index_S44(s,:)==1),3);
        
    end
    RSA_offset1_all = (RSA_offset1_all_S11 + RSA_offset1_all_S22 + RSA_offset1_all_S33 + RSA_offset1_all_S44)/4;
    
    figure(61);clf;
    imagesc(mean(RSA_offset1_all,3));
    xticks([0 0.5 1 1.5 2]*100)
    xticklabels({'0','500','1000','1500','2000'})
    yticks([0 0.5 1 1.5 2]*100)
    yticklabels({'2000','1500','1000','500','0'})
    xlabel('Episodic sequence offset [ms]')
    ylabel('Sequence encoding [ms]')
    title('RSA event with event-offset')
    caxis([-0.4,0.4])
    %% Figure 6d
    figure(65);clf;
